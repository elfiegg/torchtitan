...
_diag_dispatch_count: int = 0  # diagnostic counter for MXFP8 padding checks
...
def _dispatch_impl(
    ...
    # --- diagnostic: verify MXFP8 padding is effective ---
    global _diag_dispatch_count
    if _diag_dispatch_count < 5:
        import logging
        from torchtitan.models.moe.utils import TOKEN_GROUP_ALIGN_SIZE_M
        M = hidden.shape[0]
        logging.warning(
            "[HybridEP diag] TOKEN_GROUP_ALIGN_SIZE_M=%s  pad_multiple=%s  "
            "hidden.shape[0]=%d  %%32=%d  tokens_per_expert=%s",
            TOKEN_GROUP_ALIGN_SIZE_M, pad_multiple, M, M % 32,
            tokens_per_expert.tolist() if not (num_permuted_tokens is not None) else "non-blocking",
        )
        _diag_dispatch_count += 1
